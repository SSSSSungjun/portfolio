...
### Samsung SW·AI Academy For Youth 13기.

**Mobile Track · 2025.01 ~ 2025.12 · 수료**
...
**Android & Backend Developer · 기술 설계 · 2026.07 ~ 2026.08**
...
`Kotlin` `Jetpack Compose` `Room` `WorkManager` `Ktor Client` `Hilt` `Coroutines` `Java 17` `Spring Boot` `Spring Security` `PostgreSQL` `JWT` `Flyway` `Docker Compose` `GitHub Actions`

### 대나무숲 | 익명 웹 게시판.

**Full-stack Developer · 기술 설계 · 2026.07**
...
`Java` `Spring Boot` `Spring Data JPA` `PostgreSQL` `Flyway` `React` `TypeScript` `Axios` `Docker Compose` `Nginx` `k6` `SSE`

### Mori | 서버 리포트 Android 앱.

**Android Developer · 2025.09**
...
`Kotlin` `Jetpack Compose` `Hilt` `Coroutines` `Flow` `Retrofit2` `Canvas` `Navigation` `Multi-Module`

### 관:상견례 | 실시간 1대1 채팅 Android 앱.

**Android Developer · 2025.07.08 ~ 2025.08.21**
...
`Kotlin` `Jetpack Compose` `Hilt` `Retrofit2` `OkHttp` `WebSocket` `DataStore` `Paging3` `Navigation` `Firebase` `ML Kit`

### 싱싱과일 | 매장 주문·NFC 결제 플랫폼.

**Android & Backend Developer · 2025.05**

#### Description.

비콘 감지와 GPS 위치 확인을 연동한 매장 내 모바일 주문·결제 플랫폼을 개발했습니다.
...
`Kotlin` `XML` `MVVM` `Beacon` `GPS` `NFC` `Java` `Spring Boot` `MyBatis` `AWS EC2`

### 따숨 | 환경 공공데이터 활용 친환경 실천 Android 앱.

**Android Developer · 2025.04 ~ 2025.05**
...
`Java` `XML` `ViewModel` `Hilt` `Multi-Module`

### AI 심리 상담사 앱 | AI 해커톤.

**Android Developer · 2024.07.15 ~ 2024.07.26**
...
**Android Frontend · 2023.09.04 ~ 2023.11.16**
