# 윤성준 | Android Developer

Kotlin과 Jetpack Compose 기반 Android 네이티브 앱 개발을 주력으로 하고 있습니다. 모바일 앱의 구조 설계와 네트워크·데이터 처리 경험을 중심으로 쌓아왔으며, 서비스 전체 흐름을 이해하기 위해 Java·Spring Boot 기반 백엔드도 함께 개발했습니다.

**GitHub** https://github.com/SSSSSungjun  
**Focus** Android · Mobile Application · Backend

---

## Experience

### 한국동서발전 울산본부 | 인턴
**2026.04.06 ~ 2026.08.28**

- 정보보안 부서 업무 지원
- 정보보안 관련 자료 조사 및 문서 관리

---

## Education & Training

### 건국대학교 컴퓨터공학부 | 학사
**2018.03 ~ 2025.02**

### Samsung SW·AI Academy For Youth 13기 | Mobile Track
**2025.01 ~ 2025.12 | 수료**

- Java, Kotlin, Android, Spring Boot, MySQL, 알고리즘, Vue.js 학습
- Android 네이티브 앱 개발을 중심으로 모바일 및 백엔드 팀 프로젝트 수행

---

## Tech Stack

### Android

`Kotlin` `Jetpack Compose` `XML` `MVVM` `MVI` `UDF` `Hilt`  
`Coroutines` `Flow` `Retrofit2` `Ktor Client` `OkHttp`  
`Room` `DataStore` `WorkManager` `Paging3` `Navigation`  
`WebSocket` `Firebase` `ML Kit`

### Backend & Database

`Java` `Spring Boot` `Spring MVC` `Spring Security` `Spring Data JPA`  
`JDBC` `MyBatis` `PostgreSQL` `MySQL` `Redis` `JWT` `Flyway`

### Frontend

`React` `TypeScript` `Vue.js`

### Languages

`Kotlin` `Java` `JavaScript` `TypeScript` `Python` `C++` `SQL`

### Infra & Tools

`Docker` `Docker Compose` `Nginx` `AWS EC2` `Git` `GitHub` `GitHub Actions`  
`Jira` `Figma` `Notion` `Codex`

---

## Projects

### Bike-book | 사내 자전거 예약 Android·Backend 앱
**2026.07 ~ 2026.08 | Android & Backend Developer · 기술 설계**

- 사내 자전거의 신청, 승인, 이용, 반납 흐름을 구현하고 Android 전체 구조와 API·데이터 계약을 설계했습니다.
- Room·WorkManager 기반 오프라인 재전송과 서버 예약 충돌·중복 요청 방어, GitHub Actions 기반 자동 검수를 구성했습니다.

**Tech** `Kotlin` `Jetpack Compose` `Room` `WorkManager` `Ktor Client` `Java 17` `Spring Boot` `PostgreSQL` `GitHub Actions`

### 대나무숲 | 익명 웹 게시판
**2026.07 | Full-stack Developer · 기술 설계**

- 쿠키 기반 익명 세션으로 작성자 소유권을 검증하고 조회수·좋아요 등 동시 요청의 데이터 정합성을 처리했습니다.
- GitHub-hosted runner에서 200 VU, 3분 부하 테스트를 수행해 총 75,336건 요청, 오류율 0%, p95 52.89ms를 확인했습니다.

**Tech** `Java` `Spring Boot` `Spring Data JPA` `PostgreSQL` `React` `TypeScript` `Docker Compose` `Nginx` `k6`

### Mori | 서버 리포트 Android 앱
**2025.09 | Android Developer**

- EC2 서버의 CPU, 메모리, 디스크, 네트워크 지표를 Android 앱에서 조회하고 Canvas 기반 커스텀 차트로 시각화했습니다.
- 멀티모듈 구조를 설계하고 Coroutines `async/await`로 다중 API를 병렬 처리했습니다.

**Tech** `Kotlin` `Jetpack Compose` `Hilt` `Coroutines` `Flow` `Retrofit2` `Canvas` `Navigation`

### 관:상견례 | 실시간 1대1 채팅 Android 앱
**2025.07.08 ~ 2025.08.21 | Android Developer**

- 얼굴 분석을 활용한 1대1 사용자 매칭 및 실시간 채팅 Android 앱을 개발했습니다.
- WebSocket 메시지 송수신, DataStore 세션 관리, Paging3 기반 채팅 이력 페이징을 구현했습니다.

**Tech** `Kotlin` `Jetpack Compose` `Hilt` `Retrofit2` `OkHttp` `WebSocket` `DataStore` `Paging3` `Firebase` `ML Kit`

### 싱싱과일 | 매장 주문·NFC 결제 플랫폼
**2025.05 | Android & Backend Developer**

- 비콘·GPS 기반 매장 주문 흐름과 Android NFC API 결제 시뮬레이션을 구현하고 Spring Boot·MyBatis 백엔드를 AWS EC2에 배포했습니다.

**Tech** `Kotlin` `XML` `MVVM` `NFC` `Java` `Spring Boot` `MyBatis` `AWS EC2`

### 따숨 | 환경 공공데이터 활용 친환경 실천 Android 앱
**2025.04 ~ 2025.05 | Android Developer**

- 환경부 공공데이터를 활용한 친환경 실천 앱에서 멀티모듈 구조와 Hilt·MVVM 기반 Android 기능을 개발했습니다.

**Tech** `Java` `XML` `ViewModel` `Hilt`

### AI 심리 상담사 앱 | AI 해커톤
**2024.07.15 ~ 2024.07.26 | Android Developer**

- AI 상담 결과를 제공하는 모바일 앱에서 Android 채팅 UI와 비즈니스 로직을 담당했습니다.

### 카본풋풋 | 졸업 프로젝트
**2023.09.04 ~ 2023.11.16 | Android Frontend**

- 음식 사진을 촬영하면 탄소 발자국을 알려주는 온디바이스 AI Android 앱의 프론트엔드를 담당하고 Figma로 화면 흐름과 API 연동 지점을 정리했습니다.

---

## Certifications & Language

- **TOEIC Speaking IH (140)** | ETS | 2026.09.06 | 유효기간 2028.09.06
- **한국사능력검정시험 2급** | 국사편찬위원회 | 2026.06.05
- **SQLD** | 한국데이터산업진흥원 | 2025.09.19
- **정보처리기사** | 한국산업인력공단 | 2024.06.18

---

## Awards & Activities

### AI Worker 해커톤 최우수상(2등)
**2022 | 4인 팀**

- Arduino와 HuskyLens를 활용한 자동화 기기를 개발하고 색상 인식 로직, 코드 병합·버전 관리, 하드웨어·소프트웨어 통합을 담당했습니다.

### CS 스터디 | 조장
**2025.02.10 ~ 2025.05.23**

- Computer Science 주제를 분담해 학습하고 발표 및 질의응답을 진행하는 스터디를 운영했습니다.
